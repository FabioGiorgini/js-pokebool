...
// MILESTONE 2
// Aggiungere alle singole caratteristiche di ciascun pokemon i tipi - sotto forma di badge - e le debolezze 
// sotto forma di lista con virgole es. (Grass, Poison)

// MILESTONE 3
// Trasformare la lista delle debolezze da elenco separato da virgole a singole icone tramite lista in wicons

//MILESTONE 4
// Aggiungere un indicazione della lunghezza dei pokemon visibili prima della lista

//MILESTONE 5
// Aggiungere la logica per i bottoni di ricerca e reset. Scrivendo nell'input e premendo il bottone cerca dovrà essere aggiornata la lista
// dei pokemon visibili con solo quelli che contengono l'input inserito dall'utente.
...
// NOTA: il badge dovrà rimanere visibile anche quando si filtra la lista
...
// NOTA: i contatori devono essere aggiornati ogni volta che viene aggiornata la lista dei pokemon visibili
...
const elList = document.getElementById('poke-list');
const searchInput = document.getElementById('filter');
const resLength = document.getElementById('results');
...
const searchBtn = document.querySelector('.btn-search').addEventListener('click', ()=>{
    filter();
})
...
//emptying input and refreshing cards creation
function showInitialCards() {
    searchInput.value = '';
    refreshView();
}

//removing cards from list and readding again + refreshing found pokemons text shown
function refreshView(){
    elList.innerHTML = '';
    for(let p in list.pokemon){
        const pkm = list.pokemon[p];
        createAndPushCard(pkm);
    }
    refreshLength();
    addStats();
}
...
function filter(){
    ...
    if(input != '') {
        actualListHide = [];
        list.pokemon.map((p)=>{
            if(!p.name.toLowerCase().includes(input.toLowerCase())) actualListHide.push(p.name);
        })
        refreshView();
    }
}
...
function refreshLength() {
    const shown = list.pokemon.length - actualListHide.length;

    resLength.innerText = `Pokemon trovati: ${shown}/${list.pokemon.length}`;
}

//responsible to create li card element
function createAndPushCard(pokemon) {
    //checking if this pokemon needs to be shown actually
    if(!actualListHide.includes(pokemon.name)){
        const el = document.createElement('li');
        el.classList = 'list-group-item col-2';
        // if(pokemon.like) el.classList.add('like');
        if(favorites.includes(pokemon)) el.classList.add('like');

        //calling function to create all infos to li element
        let card = createCardContent(pokemon);
    
        //appending card to li
        el.innerHTML = card;
    
        //clicking card like badge add
        ...
        //real appending of li to ul.poke-list
        elList.appendChild(el);
    }
}

function createCardContent(pokemon){
    let txt = `
    <div class="card">
        <div class="card-body">
    ...
    for(let t in pokemon.type){
        ...
        txt +=
            `<span class="me-2 badge bg-secondary">${pokemon.type[t]}</span>`;
    }
    ...
    let weaknesses = [];
    // <!-- weaknesses -->
    for(let w in pokemon.weaknesses){
        //VERSIONE 1
        // const weak = pokemon.weaknesses[w];
...
//function to choose between text or icons for weaknesses
function appendWeaknesses(weakList, type){
    let appTxt = '';
    if(type === 'text'){
        appTxt +=
        `${weakList.join(" , ")}</div></div></div>`;

    } else if(type === 'icons') {
        appTxt += `${weakList.join(" ")}</div></div></div>`;
    }
    return appTxt;
}

//choosing icon to replace text on weakness fields
function replaceWeakWIcon(text){
    for(let i in wicons){
        const t = wicons[i].text;
        if(t.toLowerCase() === text.toLowerCase()){
            return wicons[i].icon
        }
    }
...
function updateFavoriteList(){
    const favList = favorites.map((f)=>f.name);
    if(favList.length > 0){
        favoritesList.innerHTML = `I tuoi preferiti: ${favList.join(",")}`
    } else {
        favoritesList.innerHTML = `Nessun preferito selezionato`
    }
}

function addStats(){
  const visibles = list.pokemon.filter((p)=>{
    if(!actualListHide.includes(p.name)) return p;
  });
  stats.innerHTML = '';
  const elec = visibles.filter((v)=>v.type.includes('Electric'));
  stats.innerHTML += `<p>Il numero di pokemon elettro visibile è ${elec.length}</p>`;
  const psy = visibles.filter((v)=>v.type.includes('Psychic'));
  stats.innerHTML += `<p>Il numero di pokemon psico visibile è ${psy.length}</p>`;
